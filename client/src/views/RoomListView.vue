<template>
  <div>
    ...
    <div v-else-if="treeData.length === 0" class="empty-text">{{ t('common.noData') }}</div>
    ...
    <div v-for="owner in treeData" :key="'o' + owner.id" class="tree-owner card">
      <div class="tree-row owner-row" @click="toggleOwner(owner.id)">
        ...
        <div class="tree-label">
          ...
          <span v-if="owner.name_en" class="owner-name-en">{{ owner.name_en }}</span>
        </div>
      ...
      <div v-if="expandedOwners.has(owner.id)" class="tree-children">
        <div v-for="tpl in owner.templates" :key="'t' + tpl.id" class="tree-template">
          <div class="tree-row template-row" @click="toggleTemplate(tpl.id)">
            ...
            <span class="project-type-badge" :class="'type-' + tpl.project_type">
              {{ t('enum.roomType.' + tpl.project_type) }}
            </span>
            <div class="tree-label">
              <span class="tpl-name">{{ tpl.project_name || tpl.template_name }}</span>
              <span v-if="tpl.project_name_en" class="tpl-name-en">{{ tpl.project_name_en }}</span>
            </div>
        ...
        <!-- Rooms without template -->
        <div v-if="owner.noTemplatRooms.length > 0" class="tree-template">
          <div class="tree-row template-row" @click="toggleTemplate('nt_' + owner.id)">
            <span class="tree-arrow" :class="{ open: expandedTemplates.has('nt_' + owner.id) }">▶</span>
            <span class="project-type-badge type-other">其他</span>
            <div class="tree-label"><span class="tpl-name">未分类房源</span></div>
            <span class="tree-badge">{{ owner.noTemplatRooms.length }} {{ t('owner.rooms') }}</span>
          </div>
          <div v-if="expandedTemplates.has('nt_' + owner.id)" class="tree-rooms">
            <div v-for="room in owner.noTemplatRooms" :key="'r' + room.id" class="room-row">
              <span class="room-dot" :class="'dot-' + room.status"></span>
              <router-link :to="`/rooms/${room.id}`" class="room-name">{{ room.room_name_cn }}</router-link>
              <span class="room-name-en">{{ room.room_name_en }}</span>
              <span class="room-rate">{{ formatNum(room.base_daily_rate) }}/日</span>
    ...
    <div v-if="noOwnerRooms.length > 0" class="tree-owner card">
      ...
      <div v-if="expandedOwners.has('no_owner')" class="tree-children">
        <div class="tree-rooms no-tpl-rooms">
...
function toggleOwner(id) {
  if (expandedOwners.value.has(id)) expandedOwners.value.delete(id);
  else expandedOwners.value.add(id);
  expandedOwners.value = new Set(expandedOwners.value);
}

function toggleTemplate(id) {
  if (expandedTemplates.value.has(id)) expandedTemplates.value.delete(id);
  else expandedTemplates.value.add(id);
  expandedTemplates.value = new Set(expandedTemplates.value);
}

// Build tree: owners → templates → rooms
...
const noOwnerRooms = computed(() =>
  store.rooms.filter(r => !r.owner_id)
);
...
async function loadAll() {
  ...
  try {
    ...
    // Load all templates for all owners
...
.tree-row:hover { background: var(--color-bg, #f9fafb); }
...
.owner-name-en { font-size: 0.8125rem; color: #9ca3af; margin-left: 0.5rem; }
.tpl-name { font-weight: 600; font-size: 0.9375rem; }
.tpl-name-en { font-size: 0.8125rem; color: #9ca3af; margin-left: 0.375rem; }
...
.tree-children { }
.tree-template { border-bottom: 1px solid #f1f5f9; }
.tree-template:last-child { border-bottom: none; }

.tree-rooms { padding: 0.25rem 0; }
.no-tpl-rooms { padding-left: 0; }
...
.room-rate { font-size: 0.8125rem; color: #374151; margin-left: auto; }
